...
//Get strings of the from :0:0:0:  must replace with :: if correct
static bool colapse(char * tb, const char * match)
{
    char * c=strstr(tb,match);
    if(c)
    {
        char * f=c+strlen(match)-1;
        //match :0:0:0:0: len == 9
...
void Compactformat(char * cp, unsigned val[4], int len)
{

    if ((val[0]==0) && (val[1]==0) && (Val[2]==0) &&(val[3]==0)) {*cp++=':'; *cp++=':'; *cp++=0; return; }

    char tbb[60];
    char * tb=tbb;
    sniprintf(tb,60, ":%lx:%lx:%lx:%lx:%lx:%lx:%lx:%lx:",
            ((val[0] >>16) & 0xFFFF),((val[0]) & 0xFFFF) ,
            ((val[1] >>16) & 0xFFFF),((val[1]) & 0xFFFF) ,
            ((val[2] >>16) & 0xFFFF),((val[2]) & 0xFFFF) ,
            ((val[3] >>16) & 0xFFFF),((val[3]) & 0xFFFF));

    if(!colapse(tb,":0:0:0:0:0:0:0:"))
    {
        if(!colapse(tb,":0:0:0:0:0:0:"))
        {
            if(!colapse(tb,":0:0:0:0:0:"))
            {
                if(!colapse(tb,":0:0:0:0:"))
                {
                    if(!colapse(tb,":0:0:0:"))
                    {
                        if(!colapse(tb,":0:0:"))
    ...
    //Or possible ::xxx:
    //Or :xxx::
    if(tb[1]!=':') // not State::xxx:
    {
        tb++;
    }
    int len=strlen(tb);
    if((tb[len-1]==':') && (tb[len-2]==':'))
    {len++;
    }
    tb[len-1]=0;
    strcpy(cp,tb);
...
unsigned int readbeuint32(char * cp)
{
unsigned int i=0;
i=*cp;
i=(i<<8);
...
void FieldDescription::AddrShow(unsigned char * pD, int len)
{
char buffer[80];
buffer[0]=0;
if(len==4)
{
sprintf(buffer,"%d.%d.%d.%d",pD[0],pD[1],pD[2],pD[3]);
}
else
if(len==128)
{
unsigned int val[4];
	for(int i=0; i<4; i++)
	{
		val[i]=read_beuint32(pD+(i*4));
	}
Compactformat(buffer,val,80);
}
else
{
...
if(gAction==eVerbose)  printf("%s:",name.c_str());
    switch(vtype)
    {
    case 'A': AddrShow(pD+offset,len); break;
